self.addEventListener("activate", (event) => {
  event.waitUntil(
    caches.keys()
      .then((keys) => Promise.all(keys.filter((key) => key !== "weather-ping-v5").map((key) => caches.delete(key))))
...
self.addEventListener("install", (event) => {
  event.waitUntil(
    caches.open("weather-ping-v5").then((cache) =>
  ...
  event.respondWith(
    fetch(event.request)
      .then((response) => {
        ...
        caches.open("weather-ping-v5").then((cache) => cache.put(event.request, copy));
        return response;
      })
